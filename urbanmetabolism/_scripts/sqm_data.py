...
FILE_LIST = [i for i in os.listdir('data') if i.split(
    '.')[-1] == 'xls' and i not in SKIPF]
...
def get_pop_data(census_file,
    ...
    hh_size = [int(i.split("_")[-1])
               for i in census.columns if hh_keyword in i]

    inx = [i for i in census.columns if hh_keyword in i]

    start_pop = census.loc[start_year, pop_column]
    end_pop = census.loc[end_year, pop_column]
    ...
    for i in range(2010, 2031):
        ...
        pop_dic[i] = [pop, hh, pop_cap, start_sqm_cap,
                      sqm_tot, sqm_nonres, num_nonres]
...
def get_sqm_data(data_file = 'data/Commercial.data.csv'):

    kwh_data = pd.read_csv(data_file, index_col=0)
...
def _compute_sqm(i, verbose=False, counts=False):
    ...
    for typ in table_data.columns.get_level_values(0).unique():
        if verbose:
            print("{:<25}".format(typ), end='\t')
        if typ not in SKIP:
            sqm_vector_count = table_data.loc[:, idx[typ, 'Number']]
            if counts:
                sqm_vector = sqm_vector_count
            else:
                sqm_vector = table_data.loc[
                    :, idx[typ, 'Floor Area']].div(sqm_vector_count)
...
def get_sqm(grouped=True, verbose=False, counts=False):
    ...
    for e, i in enumerate(FILE_LIST):
        sqm_data_list.append(_compute_sqm(
            i, counts=counts, verbose=verbose).mean())
